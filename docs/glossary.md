...
| **TigerTag Compatible** | The free tier: anything that *talks to* TigerTag chips — readers, apps, printers, slicers, tools. No permission needed to implement it, to say so, or to show the logo in an app, documentation or store listing. |
| **TigerTag Certified** | The licensed tier: anything that *is* a TigerTag — filament and resin manufacturers, inlay and carrier producers, machines that write TigerTag identities. Only certified partners may apply the mark **on a chip, carrier, spool or packaging**, or issue TigerTag+ signatures. See [Developer documentation](./developers/README.md) and the [trademark policy](https://github.com/TigerTag-Project/TigerTag-RFID-Guide/blob/main/TRADEMARK.md). |
